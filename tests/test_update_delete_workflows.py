...
@pytest.mark.asyncio
async def test_patch_rule_valid_partial_update(async_client: AsyncClient, auth_client: AsyncClient):
    ...
    patch_res = await auth_client.patch(f"/api/v1/rules/{rule_id}", json=patch_payload)
...
@pytest.mark.asyncio
async def test_patch_rule_invalid_field_validation(async_client: AsyncClient, auth_client: AsyncClient):
    """PATCH /rules/{rule_id} fails validation (422) if weight_points is out of bounds (1-100)."""
    list_res = await async_client.get("/api/v1/rules")
    rule_id = list_res.json()[0]["id"]

    # Invalid weight_points > 100
    patch_res = await auth_client.patch(f"/api/v1/rules/{rule_id}", json={"weight_points": 150})
    assert patch_res.status_code == 422


@pytest.mark.asyncio
async def test_delete_rule_soft_and_hard(auth_client: AsyncClient):
    ...
    create_res = await auth_client.post("/api/v1/rules", json=create_payload)
    assert create_res.status_code == 201
    rule_id = create_res.json()["id"]

    # Soft delete (default)
    soft_del_res = await auth_client.delete(f"/api/v1/rules/{rule_id}")
    assert soft_del_res.status_code == 204

    # Verify soft deleted (is_active = False)
    detail_res = await auth_client.get(f"/api/v1/rules/{rule_id}")
    assert detail_res.status_code == 200
    assert detail_res.json()["is_active"] is False

    # Hard delete
    hard_del_res = await auth_client.delete(f"/api/v1/rules/{rule_id}?soft=false")
    assert hard_del_res.status_code == 204

    # Verify rule is permanently gone (404)
    after_res = await auth_client.get(f"/api/v1/rules/{rule_id}")
...
@pytest.mark.asyncio
async def test_patch_blocklist_partial_update(auth_client: AsyncClient):
    ...
    add_res = await auth_client.post("/api/v1/blocklist", json=add_payload)
    ...
    patch_res = await auth_client.patch(f"/api/v1/blocklist/{entry_id}", json=patch_payload)
    assert patch_res.status_code == 200
    data = patch_res.json()
...
@pytest.mark.asyncio
async def test_delete_blocklist_hard_and_soft(auth_client: AsyncClient):
    ...
    # Hard delete
    del_res = await auth_client.delete(f"/api/v1/blocklist/{entry_id}?hard=true")
    assert del_res.status_code == 204

    # Detail check -> 404
    get_res = await auth_client.get(f"/api/v1/blocklist/{entry_id}")
...
async def test_delete_resolved_case_integrity_constraint(async_client: AsyncClient, auth_client: AsyncClient):
    ...
    del_res = await auth_client.delete(f"/api/v1/cases/{case_id}")
